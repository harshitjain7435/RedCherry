...
<?php
include"db.php";
$quantity=$_POST["quntity2"];
$cart_id=$_POST["id"];
if(isset($_SESSION["dummy_usr_id"]))
 {
	//$usr_email=$_SESSION["usr_email"];
	$find=mysqli_query($con,"select * from dummy_usr where id='$cart_id'");
	while($rr=mysqli_fetch_array($find))
	{
		$a=$rr[3];
		$a=$a-1;
		mysqli_query($con,"UPDATE dummy_usr SET quantity='$a' where id='$cart_id'");
	}
}
...
//$usr_email=$_SESSION["usr_email"];
	...
	while($rr=mysqli_fetch_array($find))
	{
		$a=$rr[3];
		$a=$a-1;
		mysqli_query($con,"update cart set quantity='$a' where cart_id='$cart_id'");
	}
}
